...
del act_table1, act_table2, assessment_table1,eventcnt_max,eventcnt_mean,eventcnt_std,result_table
del temp_table, temp_table2, temp_table3
...
del a_g, check1, check2,train_a, train_a_count,train_a_data,train_a_data_true
del train_acc, train_assessment, train_b, train_b_count, train_b_data, train_b_data_true,train_bird
del train_bird_count, train_else, train_else_count
...
del df1, df2, df_Assessment2, df_Assessment2_1, df_Assessment2_3, df_Clip2,feed_x6,rule,score,start_assessment_id
del temp_duration, temp_gamesession, temp_installation_id, temp_timestamp, temp_title,temp_world2
...
del G_table, G_table1,G_table2, G_table3,T_g_table, T_g_table_1,T_g_table_3, train_x7_1
...
del G_table,G_table1, G_table2, G_table3, T_g_table,T_g_table_1, T_g_table_3, train_x8_1
...
del A_table,T_a_table, T_a_table_1, T_a_table_3,a2,a_t,assessment_table1, click_train, click_train2
del click_train3,click_train4, click_train5,click_train5_x, click_train5_y,clip_list, count,dict_a, dict_act_help, dict_g,dict_game_help,dict_val
del s1,s2,s3,temp
...
del game_table, ass_table, T_table,T_table1,t1,ass_time,last_act_time, ass_act_time,ass_act_time_final
##########################################################################
#train 합치기
feed_train = real_train.loc[(real_train['type']=='Assessment') & (real_train['event_code']==2000), ['game_session']]
...
#feed_train.to_csv(home/"feed_train.csv", index=False)
#feed_train = pd.read_csv(home/"feed_train.csv")
#############################################################################
...
del T_count, T_table, T_table1,event_enter,game_table, title_mean_table, title_std_table, ass_table
...
del EC_count,T_table, T_table1,ass_table, code_mean_table, code_std_table,game_table,df_count
...
del TEC_count, T_table, T_table1, ass_table, df_count,game_table
...
del df1, df2, df_Assessment2, df_Assessment2_1, df_Assessment2_3, df_Clip2 
del temp_duration, temp_gamesession, temp_installation_id, temp_timestamp, temp_title,temp_world2
...
del G_table, G_table1,G_table2, G_table3,T_g_table, T_g_table_1,T_g_table_3, test_x7_1
...
del G_table,G_table1, G_table2, G_table3, T_g_table,T_g_table_1, T_g_table_3, test_x8_1
...
feed_test= feed_test.sort_values('timestamp_x')
...
#feed_test.to_csv(home/"feed_test.csv",index=False)
######################################################################

######################################################################
x_train = feed_train.drop(['game_session','accuracy_group'],axis=1)
x_train = np.array(x_train)

y_train =feed_train['accuracy_group']
y_train = np.array(y_train)

x_test = feed_test.drop(['game_session','installation_id'],axis=1)
...
f1 = feed_test[['installation_id']]
f1.index = np.arange(0,len(f1))
test_final = pd.concat([f1,yy],axis=1)

test_final.columns.values[-1] = 'accuracy_group'

total = test_final[['installation_id','accuracy_group']]
import math
f2 = lambda x : math.ceil(x)
total['accuracy_group'] = list(map(f2,total['accuracy_group']))
total.accuracy_group = total.accuracy_group.astype('int')
total.to_csv('submission.csv',index=False )    #  int
